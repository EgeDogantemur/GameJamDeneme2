...
public class PlayerMoverObstacle : MonoBehaviour
{
    // List of nodes (positions) the player can move between
    ...
    private void MoveToNextNode(Vector3 direction)
    {
        ...
        for (int i = 0; i < nodes.Count; i++)
        {
            if (i != currentNodeIndex)
            {
                ...
                // Check if the node is in the desired direction and there is no obstacle
                if (Vector3.Dot(direction, directionToNode) > 0.9f && !IsObstacleBetween(transform.position, nodes[i].position))
                {
